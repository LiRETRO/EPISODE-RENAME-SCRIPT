...
from utils.season_utils import get_season_cascaded, get_season, get_season_path, file_name_matcher
...
def get_season_and_ep(file_path):
    ...
    if not _:
        ...
        return None, None
    season, ep = file_name_matcher(file_name, force_rename)
    if season and ep:
...
def argument_parse():
    if len(sys.argv) > 1 and not sys.argv[1].startswith('-'):
        ...
        if parse_resolution:
            name_format = 'S{season}E{ep} - {resolution}'
    return target_path, rename_delay, rename_overwrite, name_format, name_format_bypass, force_rename, custom_replace_pair, use_folder_as_season, del_empty_folder, priority_match
...
if __name__ == "__main__":
    ...
    target_path, rename_delay, rename_overwrite, name_format, name_format_bypass, force_rename, custom_replace_pair, use_folder_as_season, del_empty_folder, priority_match = argument_parse()
    ...
    if os.path.isdir(target_path):
        logger.info(f"{'文件夹处理', target_path}")
